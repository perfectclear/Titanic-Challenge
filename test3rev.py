...
train_df.head()
#d = train_df['Survived'].groupby(train_df['FamilyName']).describe().sort_values(by=['count'], ascending=False).groupby(['count']).mean()
#d
#for dataset in combine:
#    familysurvival = dataset['Survived'].groupby(dataset['FamilyName']).describe()['mean']
#    for i in dataset['FamilyName']:
#        dataset['FamilySurvived'] = familysurvival[i]

#for j, group in train_df.groupby(['FamilyName']):
#    print(j)

#     MAYBE DO CLUSTERING? KNN?
